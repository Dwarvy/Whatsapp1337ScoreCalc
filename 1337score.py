		...
		if len(line) >= 6:
			...
			if date not in datesChecked:
				...
				if time == "13:37":
					message = line[line.find(":",  15)+1:]
					if message.strip() == "1337":
						person = line[line.find("-")+2:line.find(":",  15)]
						if person in addressBook:
							person = addressBook[person]
...
def calculateScore(lines, language):
...
def handle_docs_audio(m):
	...
	lines = str(downloaded_file).split('\\n')
	print("File Info:\n" + str(file_info) + "\n\n")
	if lines[0].find('-') < 5:
		lang = "NL"
		print("Dutch detected")
	...
	scoreBoard = calculateScore(lines, lang)
